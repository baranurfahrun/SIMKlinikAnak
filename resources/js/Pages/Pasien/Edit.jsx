...
// Komponen Sub-UI dipindahkan ke luar untuk mencegah re-mount saat pengetikan
...
export default function Edit({ auth, pasien }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        stts_nikah: pasien.stts_nikah || 'BELUM MENIKAH',
        gol_darah: pasien.gol_darah || '-',
        pendidikan: pasien.pendidikan || '-',
        no_peserta: pasien.no_peserta || '',
        email: pasien.email || '',
        suku_bangsa: pasien.suku_bangsa || '-',
        bahasa_pasien: pasien.bahasa_pasien || 'BAHASA INDONESIA',
        cacat_fisik: pasien.cacat_fisik || '-',
        kelurahan: pasien.kelurahan || '',
        kecamatan: pasien.kecamatan || '',
        kabupaten: pasien.kabupaten || '',
        propinsi: pasien.propinsi || ''
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('pasien.update', pasien.no_rkm_medis), {
            onSuccess: () => {
                Swal.fire({
                    title: 'Berhasil!',
    ...
    return (
        <AppLayout auth={auth} header="Edit Data Pasien">
            <Head title="Edit Data Pasien" />

            <div className="max-w-7xl mx-auto px-4">
                <div className="mb-8 flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800">Edit Pasien</h2>
                        <p className="text-sm text-slate-500">Perbarui informasi pasien No. RM: {pasien.no_rkm_medis}</p>
                    </div>
                    <Link
                ...
                <form onSubmit={submit} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white/80 backdrop-blur-xl rounded-[32px] shadow-sm border border-white p-8">
                            <SectionHeader icon="fa-user-edit" title="Data Utama" subtitle="Identitas dasar pasien" />
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <Label>No. Rekam Medis</Label>
                                        <Input value={data.no_rkm_medis} disabled />
                                    </div>
                                    <div>
                    ...
                    <div className="flex justify-center pt-8">
                        <button
                            type="submit"
                            disabled={processing}
                            className="w-full max-w-lg py-5 bg-gradient-to-r from-amber-600 to-amber-400 text-white font-bold rounded-[24px] shadow-xl shadow-amber-100 hover:shadow-amber-200 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 text-lg"
                        >
                            {processing ? (
                                ...
                                <>
                                    ...
                                    <span>PERBARUI DATA PASIEN</span>
                                </>
                            )}
                        </button>
                    </div>
